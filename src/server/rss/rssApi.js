...
router.delete(`${rssSubscriptionPath}/:id`, async (req, res) => {
    try {
        ...
        prisma.$transaction(async tx => {
            // 删除订阅内容
            await tx.$executeRaw`delete from main.RssSubscriptionItem where rssSubscriptionId = ${Number(id)};`
            // 删除订阅
...
router.post(rssSubscriptionItemPath, async (req, res) => {
    try {
        ...
        const items = feed.items.filter(item => {
            return item.guid && item.link && item.title && item.enclosure.type === 'application/x-bittorrent' && item.enclosure.url
        })
        const insertItems = []
...
router.put(`${rssSubscriptionItemPath}/:uuid/read`, async (req, res) => {
    try {
        ...
        const result = await prisma.$executeRaw`update RssSubscriptionItem set isRead = true, updatedAt = current_timestamp where id = ${uuid}`
        responseWithSuccess(res, null, result)
    } catch (error) {
        responseWithError(res, error);
    }
