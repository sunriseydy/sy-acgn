 ...
 * @returns {Object}
 */
export default function parseFileName(fileName: string): AnimeInfo {
  ...
  const trueName = parseResult.extensionName?.trueName ?? fileName
  ...
  if (firstBlock != null) {
    ...
    for (let groupName of firstBlock) {
      ...
      const isGroup = matchGroupName(groupName)
      // 成功解析匹配到发布组
      ...
      if (isGroup === null) {
        if (
          firstBlock.length == 1 &&
          (firstBlock[0].match(/ |\?|\!|,|？|！|，|。/) || firstBlock[0].match(/ARTE|RideBack/i))
        ) {
  ...
  for (const i in nameNoFirstBlock) {
    const isGarbage = garbageCleaner(nameNoFirstBlock[i])
    ...
    for (const i in nameNoFirstBlock) {
      ...
      if (
        ...
        for (const j in nameNoFirstBlock) {
          ...
          if (nameNoFirstBlock[j].match(/(OVA|SP|OAD|NCOP|NCED|SONG)\d{0,3}/i)) break // OVA SP 等类型到达结尾
  ...
  for (const i in nameNoFirstBlock) {
    const result = matchThisWord(nameNoFirstBlock[i])
  ...
  */
  ...
  if (parseResult.animeTitle) {
    ...
    // 查找动画标题中可能存在的年份
    const animeYear = parseResult.animeTitle.match(/(19|20)\d\d/)
    if (animeYear) {
  ...
  // 判断 noBrowser
  for (const tag of parseResult.tagedName) {
...
function matchGroupName(word) {
  // 遍历此词典的内容
  for (const group of dict.group) {
    const thisDictRegExp = group.from
    const thisDictResult = group.to
...
function matchThisWord(word) {
  const dictUsed = ['source', 'quality', 'subtitle', 'other']
  // 遍历使用的词典列表
  for (const dictName of dictUsed) {
    const thisDict = dict[dictName] // 此词典内容
    // 遍历此词典的内容
    for (const thisRule of thisDict) {
      // 如果正则将传入的 word 完全替换掉了，说明匹配成功。这时 thisWordReplaced 为 true
      const thisWordReplaced = !word.replace(thisRule.from, '').trim()
      // 匹配成功的逻辑
      if (thisWordReplaced) {
        const result = {
          result: thisRule.to,
...
function garbageCleaner(word) {
  ...
  const garbageDict = dict.delete // 垃圾正则
  for (const i in garbageDict) {
    // 遍历此词典的内容
    const thisDictRegExp = garbageDict[i]
    const thisWordReplaced = word.replace(thisDictRegExp, '').trim()
    if (!thisWordReplaced) {
      return true
    }
...
function getExtensionName(fileName) {
  ...
  const extNameWithoutDot = parsedPath.ext.replace(/^\./, '')

  for (const i in dict.format) {
    const thisDict = dict.format[i]
    const matched = extNameWithoutDot.replace(thisDict.from, '').trim()
    if (!matched) {
      return {
        result: thisDict.to,
        type: thisDict.type,
  ...
  if (extNameWithoutDot.length <= 5) return { raw: extNameWithoutDot, trueName: parsedPath.name }
  else return null
}

function tidyStringArray(list) {
  // 传入一个字符串列表，返回 trim 和删除假值后的结果
  for (const i in list) list[i] = list[i].trim() // 去除首尾空格
